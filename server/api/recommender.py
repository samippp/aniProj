...
def importAnimeDataAndProcess():
    import os
    rel_path = os.path.join(os.path.dirname(__file__) , '..', '..', 'scraper')
    file_path=  os.path.abspath(rel_path)
    ani_data = pd.read_json(file_path+'/anime_data.json')
    ani_data = ani_data[ani_data['popularity'] >= 1500]
    ani_data = ani_data[ani_data['score'] >= 5.0]

    return ani_data

def importLikedAnimeAndProcess(anime_data, all_genres):
    import json
    from io import StringIO
    liked_anime = pd.read_json(StringIO(json.dumps(anime_data)))
...
def getFavouriteGenres(anime_data):
    import json
    from io import StringIO
...
def recommend(anime_data):

    pd.set_option('display.max_rows', None) 
    pd.set_option('display.expand_frame_repr', False)

    ani_data = importAnimeDataAndProcess()
    ...
    user_profile = importLikedAnimeAndProcess(anime_data,all_genres)
    ...
    recommendations = ani_data.iloc[simularity_indexes].iloc[:300,0:7]
